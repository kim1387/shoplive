...
public class BoardRepositoryQuerydslImpl implements BoardRepositoryQuerydsl {
  ...
  @Override
  public List<BoardPreviewInfo> findBoardPreviewInfoListPage(Pageable pageable, boolean isTimeReversed) {
    return queryFactory
        .select(
  ...
  @Override
  public List<BoardPreviewInfo> getBoardListSearchTitleOrContent(Pageable pageable, boolean isTimeReversed, String keyword) {
    return queryFactory
        .select(
            new QBoardPreviewInfo(
                board.title,
                ...
                JPAExpressions.selectFrom(image).where(image.board.eq(board)).exists()))
        .from(board)
        .where(board.isRemoved.eq(false).and((board.title.contains(keyword)).or(board.content.contains(keyword))))
        .innerJoin(board.user, user)
        .offset(pageable.getOffset())
        .limit(pageable.getPageSize())
        .orderBy(boardSort(isTimeReversed))
